...
from A3C_TRADING_ENV import Micro_Arch_Live
...
def LogSummary(episode_count,action,epPercentagePriceChg,keep_prob,TSL_OFFSET):
    # Periodically save model parameters, and summary statistics.
    summary = tf.Summary()
    summary.value.add(tag='Perf/%PriceChg', simple_value=float(epPercentagePriceChg))
    summary.value.add(tag='Perf/action', simple_value=float(action))
...
summary_writer = tf.summary.FileWriter('LIVE')
...
'''
env = Micro_Arch_Live(INIT_AMOUNT,PERCENT_TO_TRADE, LEV, STOP_LOSS_PERCENTAGE, TSL_OFFSET,VolThresh,hist_lookback,mode, tDelay,loadPrevDF,USE_INIT_AMOUNT=False)
...
with tf.Session() as sess:
    ...
    for i in range(num_episodes):
        ...
        while d == False:
            ...
            a = a[0]
            s1, r, d, _ = env.step(a)
            LogSummary(i, a, r, (1 - e) + 0.1, TSL_OFFSET)
            s = s1
            '''
